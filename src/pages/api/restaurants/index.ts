...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createRestaurant() {
    ...
    if (body?.table_configuration?.length > 0) {
      const create_table_configuration = body.table_configuration;
      body.table_configuration = {
        create: create_table_configuration,
      };
    } else {
      delete body.table_configuration;
    }
